...
| `lld-plan` | Module decomposition + schema + sequenced waves (no code) | `staff-engineer` | `software-engineer`, `code-optimizer`, `dev-ops` |
| `code-diff` | Production code + tests. Implementers (`software-engineer`, `claude-code`) produce a raw diff; `code-optimizer` refines it in place. Downstream consumers always receive the optimizer's terminal `code-diff`, never the implementer's raw output. | `software-engineer` / `claude-code` → `code-optimizer` (mandatory refinement gate) | `qa-engineer`, `dev-ops`, future review agents |
...
| `qa-report` | Test execution results + structured defect / bug log (each defect routed to a responsible subagent) + ship verdict mapped to AC IDs | `qa-engineer` | feeds back to `software-engineer` / `claude-code` / `code-optimizer` (polish regressions) / `staff-engineer` / `dev-ops` (routed fix loop); terminal for humans |
...
8. **Plan revision during execution.** If a subagent's output reveals the graph is wrong (architecture turns out unsuitable; PRD is missing a requirement; the plan needs a redesign) → revise the graph: add nodes upstream, mark affected downstream artefacts unsatisfied, re-execute the affected subtree. Tell the user via `AskQuestion` before doing this when the revision changes the user-visible scope.
   - **Routed-fix loop from a `qa-report`.** When `qa-engineer` returns a `qa-report` whose defect log carries `route` fields, each open defect names the producer + artefact responsible for the fix (e.g. `software-engineer` → `code-diff`, `staff-engineer` → `lld-plan`, `dev-ops` → `deploy-artefact`, `code-optimizer` → `code-diff` for polish regressions). The parent (re)dispatches that producer for the routed defect. After a `software-engineer` / `claude-code` fix lands, the parent **re-dispatches `code-optimizer`** on the new `code-diff` before resuming the **same** `qa-engineer` (via `resume`) to re-verify. A polish-regression route (`code-optimizer` → `code-diff`) does not need a second implementer pass — optimizer, then back to QA. This is a parent-managed revision loop driven by the defect's `route` field — not a producer/consumer cycle in the graph (so it does not trip the cyclic-graph rejection in step 5). The loop continues until the verdict's blocking defects are `fixed-verified` or the user accepts the open defects.

### Mandatory optimization gate

When the dependency graph includes a `code-diff` produced by `software-engineer` or `claude-code`, the parent **must** insert a `code-optimizer` node between the implementer and every downstream consumer of that `code-diff` (`qa-engineer`, `dev-ops`, or terminal delivery to the user).

This is a **refinement edge**, not a producer/consumer cycle. Both the implementer and `code-optimizer` declare `produces: code-diff`; `code-optimizer` also `consumes: code-diff`. Do **not** treat that as a cyclic graph (step 5) and do **not** pick `code-optimizer` as the sole backward-walk producer of `code-diff` (that would recurse forever). Walk rule:

1. For a fresh implementation, the backward-walk producer of the raw `code-diff` is the implementer (`software-engineer` or `claude-code`, tie-broken by description / user hint).
2. The parent then inserts `code-optimizer` as a sequential refinement node that consumes the implementer's raw `code-diff` (plus the `lld-plan`) and produces the terminal / downstream `code-diff`.
3. Downstream consumers (`qa-engineer`, `dev-ops`, the user) wait on the optimizer's terminal output, not the implementer's.

Skip the optimizer only when:

- The parent handled the change directly (§4 step 1 trivial branch), OR
- `code-diff` is already satisfied by existing repo source (audit / verify-only tasks — no fresh implementer output).

**Fix-and-re-optimize loop.** When a `qa-report` routes a defect to `software-engineer` or `claude-code`, after that implementer's fix lands the parent re-dispatches `code-optimizer` on the new `code-diff` before resuming `qa-engineer` re-verification. A defect routed directly to `code-optimizer` (polish / footprint / formatting regression) skips the implementer and goes straight to the optimizer, then back to QA.

**Model routing.** Treat the optimizer todo as the Review / security / QA family at quality floor **Q2**, escalating to Q3 when a consolidation touches more than 10 files or a security-sensitive path.
...
- **"Why is this query slow? Fix it."** → terminal: `code-diff`. Walk back: `code-diff` ← `bug-diagnosis` ← (user-provided runtime context). Parent runs the diagnosis itself or invokes a future debugger agent, then dispatches `software-engineer`, then `code-optimizer` before terminal delivery. Three-node graph (diagnosis → implementer → optimizer); when diagnosis is parent-handled, two nodes after diagnosis.
- **"Build a notifications service"** → terminal: `code-diff` + `deploy-artefact`. Walk back: each needs `lld-plan`; `lld-plan` needs `prd` + `architecture-doc`; `architecture-doc` needs `prd`; `prd` needs `business-prompt` (user-provided). Sequential chain `product-manager → principal-engineer → staff-engineer → software-engineer → code-optimizer` then `dev-ops` (and any QA) consume the optimizer's `code-diff`.
...
- **"Here is a PRD; build the system"** → terminal: `code-diff` (+ `deploy-artefact` if runtime change). The user-attached PRD satisfies the `prd` leaf, so `product-manager` is skipped. Graph starts at `principal-engineer`; `code-optimizer` sits after the implementer (`… → software-engineer → code-optimizer`) before `dev-ops` / terminal delivery.
- **"Refactor the payment registry; here's the plan"** → terminal: `code-diff`. User-provided plan satisfies `lld-plan`. Graph is `software-engineer → code-optimizer`, not one node.
- **"Write a PRD for the new export feature"** → terminal: `prd`. Single-node graph: `product-manager`.
- **"Verify the new checkout flow"** → terminal: `test-plan` + `qa-report`. The flow is already implemented, so `code-diff` is satisfied by the repo and the `prd` / `lld-plan` provide traceability. `code-optimizer` is **absent** (repo-satisfied `code-diff`). Single-node graph: `qa-engineer`. If its `qa-report` routes a code defect back to `software-engineer`, the parent dispatches `software-engineer` then `code-optimizer` then resumes `qa-engineer` to re-verify (step 8 routed-fix loop).
- **"Build an agentic RAG service"** → terminal: `code-diff` (+ `deploy-artefact`). The task surface is AI, so the parent adds the conditional research edge: `ai-researcher` fires in **wave 1 in parallel with** `product-manager` (its `consumes` needs only the `business-prompt`), producing the `research-brief` that feeds `principal-engineer` / `ai-engineer` (architecture) and `staff-engineer` (LLD) alongside the `prd`. The rest of the walk is `… → software-engineer → code-optimizer` then `dev-ops`. Same task without an AI surface → no `ai-researcher` node at all.
- **"Design the onboarding screens"** → terminal: `ux-research` + `ux-design-spec`. Needs `prd` — if the user attached one it is satisfied; otherwise `product-manager` runs first. Single design node: `ux-designer` (which also creates the Figma files via MCP, each write relay-gated). If the ask were "design and build the onboarding screens", the graph extends: `ux-designer` (design) → `staff-engineer` (UI LLD consuming the spec) → `software-engineer` (frontend `code-diff`) → `code-optimizer`.
- **"Implement Wave 1 of the notifications service"** → terminal: `code-diff` (and `qa-report` if verification is in scope). User-provided or freshly produced `lld-plan` satisfies the plan leaf. Graph includes `software-engineer → code-optimizer → qa-engineer` (and `code-optimizer → dev-ops` after deploy-relevant waves).
...
- **"I'll skip code-optimizer because software-engineer already self-checks."** No — the optimizer is a mandatory gate after every implementer-produced `code-diff`. Implementer self-check and post-hoc optimization are complementary. Skip only on the trivial branch or when `code-diff` is already satisfied by repo source with no fresh implementer output.
...
Document-producing subagents (`product-manager`, `principal-engineer`, `staff-engineer`, `ai-engineer`, `dev-ops` in its Phase-1 plan, `qa-engineer` for its test-plan and qa-report) used to emit their whole artefact as chat output and re-render the growing document at every checkpoint. That is the dominant cause of subagent resource-exhaustion (output/context-limit) during plan / document generation, and the source of long runtimes. This section is the canonical contract that replaces it. Code-producing / refining agents (`software-engineer`, `claude-code`, `code-optimizer`) already write to files and are out of scope here.
