...
- `rules/` — user-scoped rules that ship into `~/.cursor/rules/`. Three files:
  ...
  - `plan-orchestration.mdc` — plan-time orchestration policy. When the agent produces a plan for a non-trivial task, it must also compute and embed a subagent-orchestration workflow (dependency graph with the critical path marked, dispatch waves with a width justification for every serialization, checkpoints, per-todo executor, per-dispatch time budgets) from the available subagents, per the `subagent-orchestration` skill's §10. Trivial tasks skip it.
  - `execution-time-discipline.mdc` — the universal time-optimization policy for every agent: commands are non-interactive by construction, time-boxed by runtime class, hung processes are killed and diagnosed (never re-waited), retries are bounded to 2 changed-hypothesis attempts, idle waiting and vacuous polling are banned, and exhausted budgets escalate via `AskQuestion` (parent) or a `cursor-checkpoint` `kind: blocked` block (subagent).
...
- **Todo-driven, incremental, never re-emitted.** The agent first decomposes the document into a `TodoWrite` authoring list (one todo per section), then authors strictly one section per turn — mark `in_progress` → write only that section to the file → mark `completed` → next. The todo list is the structural enforcement that prevents one-shot generation (the cause of resource-exhaustion/timeouts). Each turn returns only a short delta summary + the file path + (at a checkpoint) the `cursor-checkpoint` marker; on resume the file is edited in place, never re-printed.
...
The parent agent routes a model **per dispatched todo**, per `skills/subagent-orchestration/SKILL.md` §12. The objective is fixed: **enforce a task-specific quality floor first, then minimize expected token cost among the qualified candidates.** Key properties:

- **No permanent per-agent model pins.** Every custom agent's frontmatter is `model: inherit` (portable manual invocation, no stale provider pinning). Routed orchestration passes an explicit `model=` on each `Task` dispatch instead; model choices are recomputed per todo, never assigned permanently by agent role.
- **Live roster, dated evidence.** Candidates are the intersection of the model IDs the current `Task` schema accepts and fresh records in `skills/subagent-orchestration/model-evidence.md`. The ledger holds dated, independent, harness-comparable public-benchmark evidence plus current prices; it expires after ≤ 30 days (or immediately on a roster / price / benchmark change) and carries its refresh procedure inline. Provider claims and contaminated benchmarks (e.g. the retracted SWE-bench headline family) never establish quality; a brand-new model without independent evidence is unqualified, not preferred.
- **Validation-driven escalation, one step.** Start with the cheapest qualified model, validate through the todo's normal deterministic gate, and escalate once to the next-cheapest candidate stronger on the failed dimension. No speculative best-of-N, no premium-only collapse.
- **Token fanout warning.** Each subagent has independent context — N parallel dispatches cost roughly N× the tokens of one. Dispatch annotations carry context/output bands and the aggregate fanout band.
- **Fallback limitation.** Cursor can silently substitute a compatible model for an explicit request (admin block, plan limitation, legacy Max Mode) without surfacing the served model. Enforcement is therefore policy-time: the floor is enforced for the *requested* model, and unresolved dispatches are recorded `served-unverified` — never claimed as verified.
