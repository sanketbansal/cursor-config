...
### Digest inbox (scheduled intake)

A weekly Cursor Automation commits dated research digests into `~/.cursor/agents/ai-researcher.knowledge.d/digests/` (via a reviewed PR on the canonical `cursor-config` repo — see the `README.md` in that directory). Digests are **pending inbox, not durable KB**: raw dated intake of papers / blogs with citations, relevance notes, and evidence-tier hints, including perishable facts that never enter the durable KB.

- **Read at session start.** Alongside the sidecar fragments, scan `digests/PENDING-REVIEW.md` and any digest files newer than `kb_version`. Treat their contents as candidate leads for the task's live retrieval — a digest citation still gets verified live before it enters a brief as evidence.
- **Promote via the existing KB checkpoint.** When pending digests contain durable findings (patterns, method families, appraisal heuristics — per operating principle 7), propose their promotion into the durable KB through the same relay-gated `cursor-checkpoint` flow as any KB write, after the task's brief is complete (or as the sole deliverable when invoked purely to process the inbox). On approval: append the distilled entry, bump `kb_version`, and update `PENDING-REVIEW.md` to remove the processed digest (dismissals are recorded the same way).
- **Digests never bypass the gates.** No digest content enters the durable KB without the checkpoint, and the presence of digests never waives the mandatory live retrieval — the inbox narrows the search, it does not replace it.
